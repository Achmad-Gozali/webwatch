...
function getSecurityScore(site: WebsiteSecurity): number {
  ...
  if (site.headers) {
    ...
    const passed = keys.filter((k) => site.headers![k]).length;
    score += Math.round((passed / keys.length) * 50);
  }
  return score;
}
...
export default function SecurityPage() {
  ...
  const checkSecurity = useCallback(async (website: WebsiteSecurity) => {
    ...
    try {
      ...
      // Parse SSL info from check-website response
      const ssl: SSLInfo = {
      ...
      setWebsites((prev) =>
        prev.map((w) =>
          w.id === website.id
            ? { ...w, ssl, headers, loading: false, error: false, checkedAt: new Date().toISOString() }
            : w
        )
      );
    } catch {
  ...
  const checkAll = () => websites.forEach((w) => checkSecurity(w));
        ...
        <div className="p-4 lg:p-8 max-w-[1600px] mx-auto w-full space-y-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-zinc-500 text-sm">SSL certificate & HTTP security headers checker</p>
            </div>
            <button
              onClick={checkAll}
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="flex items-center gap-3">
                            ...
                            {score !== null && (
                              <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${getScoreBg(score)}/20 ${getScoreColor(score)} border border-current/20`}>
                    ...
                    {website.loading ? (
                              ...
                              {website.ssl.daysLeft !== null && (
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className={`text-sm font-bold ${getSSLColor(website.ssl.daysLeft)}`}>
                                      {website.ssl.daysLeft} days
                                    </span>
                                  </div>
                                </div>
                              )}
                      ...
                      <div className="flex items-center justify-center py-10 border border-dashed border-white/5 rounded-2xl gap-3">
                        ...
                        <p className="text-zinc-600 text-sm">Klik Check untuk cek keamanan website</p>
                      </div>
                    )}
