...
import {
  Zap, RefreshCw, Globe, AlertCircle, CheckCircle,
  XCircle, Monitor, Smartphone,
} from 'lucide-react';
    ...
    const loadAndAnalyze = async () => {
      ...
      if (!error && data && data.length > 0) {
        ...
        if (saved) {
          ...
          const sites: WebsitePerf[] = parsed.map((w) => ({
            ...w, scores: null, loading: true, error: false, checkedAt: null,
          }));
          setWebsites(sites);
          setLoadingWebsites(false);
        ...
        <div className="p-4 lg:p-8 max-w-[1600px] mx-auto w-full space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-zinc-500 text-sm">Lighthouse score — auto analyze saat halaman dibuka</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex items-center bg-zinc-900 border border-white/5 rounded-xl p-1">
                <button onClick={() => handleDeviceSwitch('desktop')}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${device === 'desktop' ? 'bg-emerald-500 text-white' : 'text-zinc-500 hover:text-white'}`}>
                  <Monitor className="w-3.5 h-3.5" /> Desktop
                </button>
                <button onClick={() => handleDeviceSwitch('mobile')}
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${device === 'mobile' ? 'bg-emerald-500 text-white' : 'text-zinc-500 hover:text-white'}`}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {avgScores.map((m) => (
                <motion.div key={m.key} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <p className={`text-xs font-bold mt-2 ${getScoreColor(m.avg)}`}>{getScoreLabel(m.avg)}</p>
                </motion.div>
              ))}
            </div>
          )}
          ...
          <div className="space-y-6">
            {loadingWebsites ? (
              ...
              <motion.div key={website.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                ...
                {website.loading ? (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[1, 2, 3, 4].map((i) => <div key={i} className="h-24 bg-zinc-800/50 rounded-2xl animate-pulse" />)}
                  </div>
                ) : website.error ? (
                  <div className="flex items-center gap-3 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">Google PageSpeed butuh waktu ~30 detik. Coba klik Analyze lagi.</p>
                    </div>
                  </div>
                ) : website.scores ? (
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {metrics.map((m) => {
                      ...
                      return (
                        <div key={m.key} className="bg-zinc-800/40 rounded-2xl p-4">
                          ...
                          <p className={`text-3xl font-bold mb-2 ${getScoreColor(score)}`}>{score}</p>
                          ...
                          <p className={`text-[10px] font-bold mt-1.5 ${getScoreColor(score)}`}>{getScoreLabel(score)}</p>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex items-center justify-center py-10 border border-dashed border-white/5 rounded-2xl">
                    <p className="text-zinc-600 text-sm">Sedang memuat analisis...</p>
                  </div>
                )}
